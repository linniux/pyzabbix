...
class zabbix_api: 
    def __init__(self): 
        self.url = 'http://10.16.18.5/api_jsonrpc.php' #修改URL
    ...
    def user_login(self): 
        data = json.dumps({ 
                           ...
                           "method": "user.login", 
                           "params": { 
                                      "user": "dongchangsheng", #修改用户名
                                      "password": "dongchangsheng" #修改密码
                                      }, 
                           "id": 0 
                           }) 
    ...
    def g_host_get(self,hostName='',groupid='',up=''): 
        data=json.dumps({
                "jsonrpc": "2.0",
                "method": "host.get",
                "params": {
        ...
        try: 
            ...
            available={"0":"Unknown","1":"available","2":"Unavailable"}
            #print host
            if len(hostName)==0 and up=='no':
                        #print "Host---ID : %s\t HostName : %s\t Status :\033[32m%s\033[0m \t Available :\033[31m%s\033[0m"%(host['hostid'],host['name'],status[host['status']],available[host['available']])
                        zabbix.host_disable(host['name'])
            if len(hostName)==0 and up=='yes':
                        #print "Host---ID : %s\t HostName : %s\t Status :\033[32m%s\033[0m \t Available :\033[31m%s\033[0m"%(host['hostid'],host['name'],status[host['status']],available[host['available']])
                        zabbix.host_enable(host['name'])
            if len(hostName)==0 and up=='list':
                        print "HostID : %s\t HostName : %s\t Status :\033[32m%s\033[0m \t Available :\033[31m%s\033[0m"%(host['hostid'],host['name'],status[host['status']],available[host['available']])
            #else:
                        #print "HostID : %s\t HostName : %s\t Status :\033[32m%s\033[0m \t Available :\033[31m%s\033[0m"%(host['hostid'],host['name'],status[host['status']],available[host['available']])
                #return host['hostid']
...
if __name__ == "__main__":
    ...
    parser.add_argument('-I','--gid',nargs='?',dest='listgid',default='gid',help='组id 列出分组下的所有主机')
    parser.add_argument('-U','--update',nargs='?',dest='listup',default='list',help='关闭组监控可选项yes，no')
    ...
    parser.add_argument('-e','--enable',dest='enablehost',nargs=1,metavar=('192.168.2.1'),help='启用主机')
    ...
    if len(sys.argv)==1:
        ...
        if  args.listgid != 'gid':
                zabbix.g_host_get('',args.listgid,args.listup)
        if args.listhost != 'host' and args.listgid == 'gid':
            if args.listhost:
